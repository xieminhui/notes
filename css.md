...
```
第一种不会保持图片比例，你一定会看到完整的背景图片，但是有可能被挤压(失去图片原来的比例)比如容器宽高比例是1：2，图片比例是4：3；第二种第二种效果你不一定能看到完整的图片，但是图片的比例没有发生变化（超出容器的部分被裁剪）。

### 15 fixed ios
+ fixed定位在ios下注意：滚动的页面不能跟fixed放在一起，即在同一个父元素下，否则fixed会跟着一起滚动，在定位回去。解决办法是拆分开。
+ ios下面不要将元素从无到有显示，这样做会导致元素不会立刻出现，而是等你操作完再出现，比如滚动之后才会出现你想要显示的按钮，正确的做法是设置opacity为0.0001,在设置成1。

### 16. css3开启硬件加速（GPU渲染）
+ 试用css3的3d变化可以增强动画渲染，会用gpu。