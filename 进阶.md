...
微任务的优先级是比宏任务高的，所以它会先执行。vue中dom的更新是异步的，它是用

第一段代码中，setTimeout产生宏任务，会在第二次事件循环中，promise产生微任务，会在本次事件循环中，在调用栈空了之后会执行所有的微任务，在渲染dom，一般是会渲染的，但有时也可以由浏览器决定是否在本次渲染还是等下次。而vue的dom更新是微任务，会优先执行。所以setimeout会由两次dom渲染，promise只有一次。